...
%%    EmergencyId = wh_json:get_value(<<"Outbound-Caller-ID-Number">>, OffnetReq),
    ...
    maybe_lookup_cnam(Number, OffnetReq),
...
maybe_lookup_cnam(Number, JObj) ->
    case wh_number_properties:outbound_cnam_enabled(Number) of
        'undefined' -> lager:debug("cnam for ~s is undefined Obj:~p", [Number, JObj]);
        'false' -> lager:debug("cnam for ~s no available", [Number]);
