...
%%                    lager:debug("resources sort:~p/~p", [W1, W2]),
                       W1 =< W2
               end, Resources).
            ...
            case maybe_priotity_by_lcrroute(AccountId) of
                'undefined' ->
                    case whapps_config:get_is_true(?SS_CONFIG_CAT, <<"default_lcrroute">>) of
                        'true' -> lager:debug("~p lcrroute = true", [?SS_CONFIG_CAT]), Endpoints;
                ...
                _R -> lager:debug("account lcrroute = ~s", [_R]), Endpoints
            end
    end.
...
maybe_priotity_by_lcrroute(AccountId) ->
    case kz_account:fetch(AccountId) of
        ...
        {'ok', Doc} ->
            case wh_json:get_value(<<"priotity_by_lcrroute">>, Doc) of
                'undefined' -> 'undefined';
                'true' -> 'true';
                _R -> lager:debug("account lcrroute = ~s Doc:~p", [_R, Doc]), 'false'
            end
    end.
...
get_global_endpoints(Number, OffnetJObj) ->
    ...
    Flags = wapi_offnet_resource:flags(OffnetJObj, []),
    Resources = filter_resources(Flags, get(Number)),
    resources_to_endpoints(Resources, Number, OffnetJObj).
